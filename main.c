...
static void apply_css(void){
    ...
    gtk_css_provider_load_from_string(p,
        ...
        "  font-size:18px;padding:9px 12px;min-width:0;}"
        ...
        ".login-hint{color:#94a3b8;font-size:11px;}"

        /* ── treeview — always dark text ── */
        "treeview{background:white;font-size:14px;color:#1e293b;}"
        "treeview header button{background:#f8fafc;font-size:12px;font-weight:600;"
        ...
        "  padding:6px 8px;font-size:13px;background:white;color:#1e293b;}"
        ...
        /* ── Modern dark titlebar ── */
        "window{"
        "  background:#f1f5f9;"
        "}"
        /* HeaderBar / titlebar — dark like modern apps */
        "headerbar{"
        ...
        "}"
        "headerbar .subtitle{color:#94a3b8;}"
...
static void activate(GtkApplication *app,gpointer ud){
    ...
    GtkWidget *htitle=gtk_label_new("🎓  Student Management System");
    gtk_widget_add_css_class(htitle,"title");
    gtk_header_bar_set_title_widget(GTK_HEADER_BAR(hbar),htitle);
    gtk_window_set_titlebar(GTK_WINDOW(main_window),hbar);
